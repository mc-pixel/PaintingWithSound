...
HOscillator masterRX, masterRY, masterRZ;
...
void setup() {
	...
	layout = new HSphereLayout().loc(layoutStartX,layoutStartY,layoutStartZ).radius(layoutRadius).ignorePoles();
	...
	for (int i = 0; i < numAssets; ++i) {
		...
		oscS[i]  = new HOscillator().range(5, 100).speed(1).freq(5).currentStep(i*0.1).waveform(H.SINE);
	}
