...
String      whichImg       = pathAssets + "colors_003.png";
PImage      clrs;
int         clrsW;
float       clrCount;
...
String[]    texNames       = { "fadeSquare.png" };
...
int         gridCols       = 12; // how many columns
int         gridRows       = 55; // how many rows
int         gridTotal      = gridCols * gridRows;
int         gridSpaceX     = ((int)boxW)+10; // spacing between cells on x
int         gridSpaceY     = ((int)boxH)+1; // spacing between cells on y
...
HOscillator[] oscA         = new HOscillator[gridTotal];
HOscillator[] oscH         = new HOscillator[gridTotal];
...
void setup() {
	...
	for (int i = 0; i < gridTotal; ++i) {
		...
		oscA[i]  = new HOscillator().range(-255, 255).speed(1).freq(5).currentStep( (int)random(999) ).waveform(H.SINE);
		oscH[i]  = new HOscillator().range(-boxH, boxH*5).speed(1).freq(5).currentStep( (int)random(999) ).waveform(H.SINE);
	}
}
...
// ************************************************************************************
	...
	push();
		...
		for (int i = 0; i < gridTotal; ++i) {
			...
			float _aA = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.0, 5.0);
			_oscA.speed(_aA);
			_oscA.nextRaw(); 

			HOscillator _oscH = oscH[i];
			float _aH = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, -0.1, 10.0);
			_oscH.speed(_aH);
			_oscH.nextRaw(); 
			...
			push();
				...
				buildCube(tex[0], boxW, _oscH.curr() );
			pop();
		}
	pop();
...
void buildCube(PImage _t1, float _w, float _h) {
	strokeWeight(0);
	noStroke();
	...
	beginShape(QUADS);
		texture(_t1);
		vertex(-(_w/2), -(_h/2),   0, 0); // x, y, z, u, v
		vertex( (_w/2), -(_h/2),   1, 0); // x, y, z, u, v
		vertex( (_w/2),  (_h/2),   1, 1); // x, y, z, u, v
		vertex(-(_w/2),  (_h/2),   0, 1); // x, y, z, u, v
	endShape(CLOSE);
}
