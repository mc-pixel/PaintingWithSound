...
// *********************************************************************************************
 ...
 public void setup() {
	...
	for (int i = 0; i < numAssets; ++i) {
		...
		oscZ[i]  = new HOscillator().range(-2000, 100).speed(1).freq(5).currentStep(i*2.0f).waveform(H.SINE);
...
// ************************************************************************************
	...
	push();
		...
		for (int i = 0; i < numAssets; ++i) {
			...
			float _aS = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.05f, 0.5f);
			_oscS.speed(_aS);
			_oscS.nextRaw(); 

			HOscillator _oscRZ = oscRZ[i];
			float _aRZ = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.1f, 0.3f);
			_oscRZ.speed(_aRZ);
			_oscRZ.nextRaw(); 
