...
String[]    texNames       = { "square.png", "art_001.png", "art_005.png", "particle_02.png", "rings.png", "star.png" };
...
int[]       whichTex       = new int[gridTotal]; // 0-5
...
void setup() {
	...
	for (int i = 0; i < gridTotal; ++i) {
		...
		// 		LETS RANDOMLY PICK A TEXTURE TO USE ON ALL 6 SIDES
		whichTex[i] = (int)random(texNamesLen);
	}
}
	...
	push();
		...
		for (int i = 0; i < gridTotal; ++i) {
			...
			push();
				...
				buildCube(tex[ whichTex[i] ], tex[whichTex[i]], tex[whichTex[i]], tex[whichTex[i]], tex[whichTex[i]], tex[whichTex[i]]);
			pop();
		}
	pop();
