...
// *********************************************************************************************
 ...
 public void setup() {
	...
	lightX = new HOscillator().range(-1000, 1000).speed(0.1f).freq(4).waveform(H.SINE);
	lightY = new HOscillator().range(-1000, 1000).speed(0.1f).freq(8).waveform(H.SINE);
	lightZ = new HOscillator().range(-1000, 1000).speed(0.1f).freq(10).waveform(H.SINE);
}
 ...
 public void draw() {
	...
	int c1 = clrs.get((int)waveMap,0);          // get the color from image 1
	int r1 = (c1 >>> 16) & 255;                 // grab just the RED value from this color
	int g1 = (c1 >>>  8) & 255;                 // grab just the GREEN value from this color
	int b1 = c1 & 255;                          // grab just the BLUE value from this color
...
// ************************************************************************************
	...
	push();
		...
		for (int i = 0; i < gridTotal; ++i) {
			...
			float _aZ = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.0f, 2.0f);
			_oscZ.speed(_aZ);
			_oscZ.nextRaw(); 
