...
void setup() {
	...
	for (int i = 0; i < numAssets; ++i) {
		...
		oscZ[i]  = new HOscillator().range(-2000, 100).speed(1).freq(5).currentStep(i*2.0).waveform(H.SINE);
		oscS[i]  = new HOscillator().range(5, 600).speed(1).freq(10).currentStep(i*5).waveform(H.SINE);
