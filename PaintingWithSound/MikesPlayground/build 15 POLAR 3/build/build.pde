...
// *********************************************************************************************
	//testing ground for the effect swithig function
	/*int cooldowncnt = 0; 
	Function[] magic = new Function[]{
		new magic() {drawMusic();},
		new magic() {drawPolarWindow();}
	};*/
// *********************************************************************************************
...
void setup() {
	...
	masterRX = new HOscillator().range(180, -180).speed(1).freq(1).waveform(H.SINE);
...
//HERE I DRAW THE FUCNTIONS
	/*if(myAudioData[0]>=90 && cooldowncnt ==0){
		int cnt = 1;
		if (cnt == 1) {
			(magic[1]+";");
			cnt--;
...
void drawPolarWindow(){
	...
	float _oscRX = map(myAudioData[4], 0, myAudioMax, 0.1, 2.0);
	masterRX.speed(_oscRX);
	masterRX.nextRaw();
