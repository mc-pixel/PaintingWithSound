...
int         myAudioCounter     = 0;
int         myAudioCountMax    = myAudioRange/8; // 2 / 4 / 8 / 16
boolean     myAudioCountToggle = false;
		...
		layer1.push();
			...
			for (int i = 0; i < numAssets; ++i) {
				layer1.push();
					...
					if(myAudioCountToggle) {
						myAudioCounter--;
						if(myAudioCounter<=0) {
							myAudioCountToggle = false;
							myAudioCounter = 0;
						}
					} else {
						myAudioCounter++;
						if(myAudioCounter>=myAudioCountMax) {
							myAudioCountToggle = true;
							myAudioCounter = myAudioCountMax;
						}
					}

					int hMap = (int)map(myAudioData[ myAudioCounter ], 0, myAudioMax, rectW,  300);
