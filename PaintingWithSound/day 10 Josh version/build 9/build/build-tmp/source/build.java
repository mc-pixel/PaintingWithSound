...
float       clrSpeed1        = 0.05f; // the speed of the color change
float       clrOffset1       = 0.0025f; // the distance from each of the squares getting colored

float       clrSpeed2        = 0.005f; // the speed of the color change
float       clrOffset2       = 0.0005f; // the distance from each of the squares getting colored
...
// *********************************************************************************************
 ...
 public void setup() {
	...
	masterP = new HOscillator().range(1.25f, 4.0f).speed(0.2f).freq(5).waveform(H.SINE);
}
...
// ************************************************************************************
	...
	float _MS = map(myAudioData[ myPickedAudio[0] ], 0, myAudioMax, 0.05f, 0.75f);
	masterP.speed(_MS);
	masterP.nextRaw();
	...
	push();
		...
		for (int i = 0; i < gridTotal; ++i) {
			...
			float _aX = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.0f, 1.0f);
			_oscX.speed(_aX);
			_oscX.nextRaw(); 

			HOscillator _oscY = oscY[i];
			float _aY = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.0f, 1.0f);
			_oscY.speed(_aY);
			_oscY.nextRaw(); 

			HOscillator _oscZ = oscZ[i];
			float _aZ = map(myAudioData[ myPickedAudio[i] ], 0, myAudioMax, 0.1f, 1.0f);
			_oscZ.speed(_aZ);
			_oscZ.nextRaw(); 
