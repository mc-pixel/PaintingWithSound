...
void setup() {
	...
	for (int i = 0; i < gridTotal; ++i) {
		...
		oscX[i]  = new HOscillator().range(-300, 300).speed(1).freq(1).currentStep(i*7).waveform(H.SINE);
		oscY[i]  = new HOscillator().range(-300, 300).speed(1).freq(1).currentStep(i*6).waveform(H.SINE);
