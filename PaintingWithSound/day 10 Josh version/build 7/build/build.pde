...
int         gridSpaceX       = 1000; // spacing between cells on x
int         gridSpaceY       = 1000; // spacing between cells on y
int         gridSpaceZ       = 1000; // spacing between cells on z
...
void setup() {
	...
	for (int i = 0; i < gridTotal; ++i) {
		...
		oscX[i]  = new HOscillator().range(-100, 100).speed(1).freq(3).currentStep(i*1).waveform(H.SINE);
		oscY[i]  = new HOscillator().range(-100, 100).speed(1).freq(4).currentStep(i*2).waveform(H.SINE);
		oscZ[i]  = new HOscillator().range(-100, 100).speed(1).freq(5).currentStep(i*1).waveform(H.SINE);
		...
		oscS[i]  = new HOscillator().range(150, 600).speed(1).freq(5).currentStep(i*3).waveform(H.SINE);
	}
...
// ************************************************************************************
	...
	push();
		...
		for (int i = 0; i < gridTotal; ++i) {
			...
			float _aS = map(myAudioData[1], 0, myAudioMax, 0.0, 2.0);
			_oscS.speed(_aS);
			_oscS.nextRaw(); 
